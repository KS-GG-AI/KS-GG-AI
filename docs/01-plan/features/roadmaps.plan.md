...
| --- | --- | --- | --- | --- | --- |
| [x] | TODO-01 | TypeScript 로드맵 빌드 기반을 추가한다. | `package.json`, `tsconfig.roadmaps.json`, `.gitignore` | Node 22에서 TypeScript 컴파일이 성공한다. | 없음 | REQ-01, REQ-07 |
| [x] | TODO-02 | 공개 API 수집·정규화·SVG·JSON 생성기를 작성한다. | `scripts/roadmaps/update-roadmaps.ts` | 공개 이슈 라벨에서 두 SVG와 상태 JSON을 생성한다. | TODO-01 | REQ-01~REQ-04, REQ-06, REQ-07 |
| [x] | TODO-03 | 생성기 계약과 트랜잭션을 검증하는 테스트를 작성한다. | `scripts/roadmaps/test-roadmaps.ts` | 분류·재시도·빈 상태·README·출력 보존 테스트가 통과한다. | TODO-02 | REQ-02~REQ-08 |
| [x] | TODO-04 | 두 로드맵 자산과 10개 언어 접기 패널을 연결한다. | `assets/`, `data/`, `README*.md` | 두 SVG가 로컬에 있고 모든 README가 같은 정보 구조를 가진다. | TODO-02 | REQ-04~REQ-06 |
...
- `npm ci --ignore-scripts`, `npm run roadmaps:test`, `node scripts/test-project-map.mjs`, `npm audit --omit=dev --audit-level=high`를 통과했다.
