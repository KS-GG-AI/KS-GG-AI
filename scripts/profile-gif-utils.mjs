...
export function createFramePainter(width, height, defaultScale = 4) {
  ...
  function drawGlyph(frame, char, x, y, color, glyphScale = defaultScale) {
  ...
  function drawText(frame, text, x, y, color, glyphScale = defaultScale) {
    let cursor = x;
  ...
  function createFrame(color = 0) {
    return new Uint8Array(width * height).fill(color);
  }

  return { createFrame, fillRect, fillRoundedRect, drawText };
}
